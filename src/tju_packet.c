...
tju_packet_t* create_packet(uint16_t src, uint16_t dst, uint32_t seq, 
    ...
    new->header.flags = flags; //标志位 比如 SYN FIN ACK 等
    new->header.advertised_window = adv_window; //建议窗口大小 用于流量控制
    new->header.ext = ext; //额外的数据 为了将 header 的大小凑整到 20bytes 没有实际意义
...
char* create_packet_buf(uint16_t src, uint16_t dst, uint32_t seq, uint32_t ack,
    ...
    final = packet_to_buf(temp); //将创建的tju_packet_t结构转化为字符串形式
...
uint16_t get_src(char* msg){
    ...
    return ntohs(var); //ntohs函数将网络字节序转换为主机字节序
}
//从数据包的字符串中提取目的端口字段
...
char* packet_to_buf(tju_packet_t* p){
    ...
    //如果包的总长度大于头部长度，则表明包中有数据部分，将数据部分复制到字符串末尾
